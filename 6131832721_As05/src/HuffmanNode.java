public class HuffmanNode implements Comparable {
	...
	public static HuffmanNode buildHuffmanTree(Heap h) throws Exception {
		...
		for(int i=0;i<n;i++) {
			//pop 2 least values
			min1 = (HuffmanNode) h.pop();
			min2 = (HuffmanNode) h.pop();
			sumf =min1.frequency+min2.frequency;
			HuffmanNode temp= new HuffmanNode(sumf,' ',min1,min2);
			h.add(temp);
		}
		return new HuffmanNode(sumf,' ',min1,min2);
	}
